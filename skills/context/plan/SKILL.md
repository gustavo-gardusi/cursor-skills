...
**Where it runs:** The **current Cursor workspace** — the git repo and files you mean to change (often the same project whose **`.cursor/research-context.json`** you built with **`@context-add`**).
...
> **`@gh-pr`** — **`@gh-pull`** → **`@gh-push`** → create/update PR (full branch vs **`main`** / **`upstream/main`**).
