...
**Goal:** Clear the research context and visited set so the next **context-add** run starts fresh. Remove or truncate **`.cursor/research-context.json`** and **`.cursor/research-visited.txt`**. Does not modify the repo or **`.cursor/research-plan.md`** unless the user asks to clear the plan too.
...
1. If **`.cursor/research-context.json`** exists: delete it or overwrite with `{ results: [], lastFetched: null }`.
