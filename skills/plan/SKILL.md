...
# Plan (read context + repo; write plan only)

**Goal:** **Mostly read.** Read the **summary of context** from **`.cursor/research-context.json`** (written only by **search**) and explore the **current repo** (read-only). From that, **craft plans**: what can be done, and what to do (concrete steps with file paths). Store the result in **`.cursor/research-plan.md`**. **Do not change the context file**—only read it. If you run again, **re-evaluate and write a new plan** (overwrite the plan file only). Does not modify the repo.
...
**Output:** **`.cursor/research-plan.md`** only. **execute** will read this file to apply the plan.

---

## Context file shape (read-only)

**search** writes `.cursor/research-context.json`. Expect:
...
Use the **summary** of the context: extract themes, requirements, APIs, constraints from `results[].title` and `results[].text`. Do not write to this file; only **search** may change it.

---

## Do not change the context file or the repo

**plan** must **never** write to or modify `.cursor/research-context.json`. Only **search** may change the context file. Plan only **reads** the context (and the repo) and **writes** the plan file. It does not add, edit, or delete any repo source files or config. When re-running, overwrite **`.cursor/research-plan.md`** only.
...
Write markdown so **execute** (and humans) can parse chunks reliably. Structure:
...
- **execute** will look for sections `## Relation to repo`, `## Impact summary`, `## Implementation plan` and parse `### Chunk N` blocks with the three bullet types (Run/one-off, Add/change, Update/wire).
...
- [ ] Next: **execute** reads the plan and applies changes to the repo.

---

## Notes

- **Workflow:** **search** (only skill that changes context) → **plan** (reads context + repo, writes plan only) → **execute** (reads plan, changes repo only).
