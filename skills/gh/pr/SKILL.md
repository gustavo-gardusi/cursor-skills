...
1. **[`@gh-pull`](../pull/SKILL.md)** (`/gh-pull`) — **Full** skill: **§1 Branch** through **§6 Done** — fetch **`origin`** (and **`upstream`** if configured), merge **`@{u}`** when present, set **`ROOT_BRANCH`**, merge canonical **`main`**, **resolve conflicts**, complete merge commits. **No** `@gh-push`, **no** `@gh-check`, **no** `git push`.
2. **[`@gh-push`](../push/SKILL.md)** (`/gh-push`) — **Full** skill **§1 → §3**: **§1** = **entire** **[`@gh-check`](../check/SKILL.md)** (`/gh-check`) in order; **§2** docs alignment; **§3** commit if needed + `git push` / `-u`.
...
> **Run the full Cursor skill [`@gh-pull`](../pull/SKILL.md)** — invoke **`/gh-pull`** and execute it **end-to-end** (not a subset of git commands you invent).
>
...
> **Run the full Cursor skill [`@gh-push`](../push/SKILL.md)** — invoke **`/gh-push`** and execute **§1 → §2 → §3** in order.
>
> **Checklist:**
> - **§1** — Run **complete** **[`@gh-check`](../check/SKILL.md)** (`/gh-check`): discover → prepare → evaluate (format/lint/test per repo). **No** `git commit` / `git push` before this finishes successfully.
