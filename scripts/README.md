...
- **`.cursor/research-context.json`** — Written **only by context-add** (fetch output). Canonical store for skills. **context-plan** reads this (read-only). Use `--append` to merge new results.
- **`.cursor/research-context.txt`** — Human-readable export (one block per page, with spacing and LINKS). Generated by **context-add** after fetch via `node scripts/context/write-readable.js`. For review; skills use the JSON.
