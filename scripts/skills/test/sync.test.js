...
import {
  ...
  usage,
  run,
  rewriteSkillLinksForInstall,
  rewriteSkillLinksForOut,
} from '../sync.js';
...
describe('skill-sync processContent', () => {
  ...
  test('rewrites inter-skill markdown links for flat install when skillsDir passed', () => {
    const skillsDir = mkdtempSync(join(tmpdir(), 'sk-links-'));
    mkdirSync(join(skillsDir, 'gh', 'pr'), { recursive: true });
    mkdirSync(join(skillsDir, 'gh', 'pull'), { recursive: true });
    writeFileSync(join(skillsDir, 'gh', 'pull', 'SKILL.md'), '# pull', 'utf8');
    const raw = 'See [pull](../pull/SKILL.md)';
    const out = processContent(raw, '/tmp/repo', 'gh/pr/SKILL.md', skillsDir);
    assert.ok(out.includes('](../gh-pull/SKILL.md)'), out);
    assert.ok(!out.includes('](../pull/SKILL.md)'), out);
    rmSync(skillsDir, { recursive: true });
  });

  test('rewriteSkillLinks round-trip restores repo-relative paths', () => {
    const skillsDir = mkdtempSync(join(tmpdir(), 'sk-rt-'));
    mkdirSync(join(skillsDir, 'context', 'execute'), { recursive: true });
    mkdirSync(join(skillsDir, 'gh', 'check'), { recursive: true });
    writeFileSync(join(skillsDir, 'gh', 'check', 'SKILL.md'), '# c', 'utf8');
    const raw = 'Run [`@gh-check`](../../gh/check/SKILL.md)';
    const installed = rewriteSkillLinksForInstall(raw, 'context/execute/SKILL.md', skillsDir);
    assert.ok(installed.includes('](../gh-check/SKILL.md)'), installed);
    const back = rewriteSkillLinksForOut(installed, 'context/execute/SKILL.md', skillsDir);
    assert.strictEqual(back.trim(), raw.trim());
    rmSync(skillsDir, { recursive: true });
  });
