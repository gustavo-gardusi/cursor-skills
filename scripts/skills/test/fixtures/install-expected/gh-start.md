...
**Cursor skill:** **`@gh-start`** — Invoked with **`@gh-start`** in Cursor. **Depends on [`@gh-main`](../gh-main/SKILL.md)** and **[`@gh-push`](../gh-push/SKILL.md):** run the **full** **`@gh-main`** skill **before** **`git checkout -b`**—**do not** reimplement checkout/fetch/merge of `main` here. Branch **naming** and **`git checkout -b`** are **`@gh-start`** only. **Always** finish with **full** **`@gh-push`** so the new branch exists on the remote (unless the user explicitly cancels publish—then report and stop before push).
...
*`@gh-start`* — Run steps in order. If the user did not give a ticket, issue, or activity, ask: "What's the Jira ticket, GitHub issue, or activity for this branch?" If you are on **`main`** with **uncommitted changes**, **`@gh-main`** will **not** stash: user must **commit**, **abort**, or **explicitly confirm trash** inside **`@gh-main`**. Prefer **commit** before **`@gh-start`** if they need to keep the work.
...
> **Run the full Cursor skill [`@gh-main`](../gh-main/SKILL.md)** — not a shortcut.  
> Execute **every** step of **`@gh-main`**: validate → checkout **`main`** first (no fetch/reset before) → **one fetch** → set **`CANONICAL_MAIN`** → fast path or dirty handling → **`git reset --hard`** + **`git clean`** (no **`git pull`** after reset—see that skill). **`@gh-main`** does **not** run **`@gh-push`**.  
> If **`@gh-main`** aborts (e.g. user declines destructive reset, missing **`upstream`** on a fork, ref does not resolve), **stop** **`@gh-start`** and report; do not create the branch until **`main`** is healthy.
...
> **Run the full Cursor skill [`@gh-push`](../gh-push/SKILL.md)** — not a shortcut.  
...
- For a PR: **`@gh-pr`** (**`@gh-pull`** → **`@gh-push`** → PR text). For **verify only** (no push): **full** **`@gh-check`**.
