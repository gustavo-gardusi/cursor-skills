...
**Where it runs:** The **current Cursor workspace** (whatever project is open). Fetched pages and **`.cursor/research-context.json`** are stored **under that workspace**, not in the skills repo.

**Scripts path:** Commands use **`__REPO_ROOT__/scripts/url`**, **`__REPO_ROOT__/scripts/context`**, etc. After **`sync in`**, those tokens are **absolute paths** into **your local clone of the cursor-skills repository** (where you ran sync). Install Node dependencies on **that** clone (**`npm install --prefix scripts`**) so **`fetch.js`** and Playwright run. **How to clone, sync, and test** that repo lives in its **README**—this skill does not duplicate setup.
...
- **`.cursor/research-context.txt`** — Generated from JSON with `node __REPO_ROOT__/scripts/context/write-readable.js`.
...
2. **Open and observe** — Start a dedicated browser session (or attach intentionally if needed) and run fetch in observer mode.
   ...
   - `node __REPO_ROOT__/scripts/url/fetch.js --observe --observe-ms 0 --observe-interval 2000 --links --links-limit 15 --visited-file .cursor/research-visited.txt URL`
...
4. **Append done pages** — For destination pages, run one final non-observe pass with `--out` so the final JSON has the expected result shape, then pass that JSON to **`append-result.js`**.
   ...
   - `node __REPO_ROOT__/scripts/url/fetch.js --links --links-limit 50 --out /tmp/fetch-out.json <destination url>`
...
Script path: **`__REPO_ROOT__/scripts/url`** (replaced at install with the actual repo path).
...
```bash
node __REPO_ROOT__/scripts/url/fetch.js --observe --observe-ms 0 --observe-interval 2000 --observe-max-ms 600000 --links --links-limit 15 --wait-after-load 3000 --visited-file .cursor/research-visited.txt --failed-file .cursor/research-failed.txt --retries 3 --observe-close-on-destination --compact URL
```
Use a different browser family when needed:
```bash
BROWSER_CHANNEL=chromium node __REPO_ROOT__/scripts/url/fetch.js --observe --observe-ms 0 --observe-close-on-destination --compact URL
node __REPO_ROOT__/scripts/url/fetch.js --browser-channel firefox --observe --observe-ms 0 --observe-close-on-destination --compact URL
```
...
```bash
node __REPO_ROOT__/scripts/context/append-result.js --file /tmp/fetch-out.json
# or: echo '{"results":[...]}' | node __REPO_ROOT__/scripts/context/append-result.js
```

**After run, write readable .txt:**
```bash
node __REPO_ROOT__/scripts/context/write-readable.js
```
